      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          ...
          <!-- 
            ...
            即会传递数据currentIndex，又会绑定更新数据的事件 update:currentIndex
           -->
          <ImageList
    ...
    async addCart() {
      try {
        ...
        /* 
          Cookie
            特点：
              1. 存储容量和大小有限制 4kb 20个左右
              2. 发送请求会自动携带上，不需要做任何操作
              3. 有过期时间，到期了会自动清除
              4. 兼容性很好

          Session
            特点：
              1. 其他数据（cookie、storage等）都是存储在客户端，session数据存储在服务端
              2. 存储容量理论上无限大的
              3. 借助cookie实现
          ...
          SessionStorage
            特点：  
              1. 会话存储，关闭浏览器会自动清除，不关浏览器会一直保留
              2. 单个存储容量 2mb左右

              注意：只能存储字符串格式数据
              使用：
                1. 读取： xxxStorage.getItem(key)
                2. 设置： xxxStorage.setItem(key, value)
                3. 删除单个： xxxStorage.removeItem(key)
                4. 删除所有： xxxStorage.clear()
          ...
          PWA（Service Workers + Cache）离线可访问
            特点：
              1. 存储容量理论上无限大（很大）
              2. 其他数据一旦断网了都不可以访问，
                  而PWA即时断网了也可以访问存储的数据
